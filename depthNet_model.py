...
class depthNet(nn.Module):
    ...
    def getVolume(self, ref_image, neighbor_image, KRKiUV_T, KT_T, image_widtha, image_heighta):

        idepth_base = 1.0 / 50.0
        idepth_step = (1.0 / 0.5 - 1.0 / 50.0) / 63.0

        costvolume = Variable(  # volume inititated of size [1, 64, 256, 320]
            torch.cuda.FloatTensor(ref_image.shape[0], 64,
                                   ref_image.shape[2], ref_image.shape[3]))

        batch_number = ref_image.shape[0]

        normalize_base = torch.cuda.FloatTensor(
            [image_widtha / 2.0, image_heighta / 2.0])
        normalize_base = normalize_base.unsqueeze(0).unsqueeze(-1)   # size [160, 128]

        for depth_i in range(64):
            # depth_i starts from 0. so first this depth = 1.0 / (idepth_base)
            this_depth = 1.0 / (idepth_base + depth_i * idepth_step)
            # size of KRKiUV_T: torch.Size([1, 3, 81920]), KT_T:torch.Size([1, 3, 1])
            # size of transformed : [1, 3, 81920]. All pixels are transformed
            ...
            warp_uv = warp_uv.view(
                batch_number, 2, image_widtha,
                image_heighta)  # shape = batch x 2 x width x height

            warp_uv = Variable(warp_uv.permute(
                0, 3, 2, 1))  # shape = batch x height x width x 2
            warped = F.grid_sample(neighbor_image, warp_uv)

            costvolume[:, depth_i, :, :] = torch.sum(
                torch.abs(warped - ref_image), dim=1)

        return costvolume

    def getdepth(self, images,  plane_sweep_volume):
        x = torch.cat((images,  plane_sweep_volume), 1)
    ...
    def forward(self, images, KRKiUV_T, KT_T, image_width, image_height, ref_idx, indices):
        cost_volume = []
        disp1 = []
        disp2 = []
        disp3 = []
        ...
        for i in indices:
            cur_idx = int(i)
            cost_volume.append(self.getVolume(
                images[ref_idx], images[cur_idx], KRKiUV_T[count], KT_T[count], image_width, image_height))
            if count == 0:
                pass
            else:
                cost_volume[count] = (cost_volume[count] + cost_volume[count-1])

            disp = self.getdepth(images[ref_idx], cost_volume[count]/(count + 1))
            disp1.append(disp[0])
            disp2.append(disp[1])
            disp3.append(disp[2])
            disp4.append(disp[3])
